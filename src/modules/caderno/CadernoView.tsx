...
import {
  ...
  useAdicionarAcao,
  useRemoverAcao,
  useAvancarFaseProjeto,
} from '@modules/projeto';
...
export function CadernoView({ status }: { status: StatusProjeto }) {
  ...
  const percentFaseAtual =
    bloqueio.totalRequisitos > 0 ? (bloqueio.cumpridos / bloqueio.totalRequisitos) * 100 : 0;
  ...
  return (
    <div style={cadernoStyle}>
      ...
      <div style={headerSection}>
        ...
        <div style={teamGrid}>
          <div style={teamItem}>
            <span style={teamLabel}>Líder</span>
            <span style={teamName}>{lider?.nome ?? '—'}</span>
          </div>
          <div style={teamItem}>
            <span style={teamLabel}>Facilitador</span>
            <span style={teamName}>{facilitador?.nome ?? '—'}</span>
          </div>
          <div style={teamItem}>
            <span style={teamLabel}>Equipe</span>
            <span style={teamName}>
              {membros.length > 0 ? membros.map(m => m.nome).join(', ') : '—'}
            </span>
          </div>
          <div style={teamItem}>
      ...
      {bloqueio.pendentes.length > 0 ? (
        <div style={avancoBlockStyle}>
          ...
          <div style={avancoFooter}>
            {bloqueio.pendentes.length} pendência{bloqueio.pendentes.length > 1 ? 's' : ''} para
            fase {projeto.faseAtual + 1}
          </div>
        </div>
      ) : (
        <div style={avancoReadyStyle}>
          ...
          <button
            onClick={() => {
              if (confirm(`Avançar para Fase ${projeto.faseAtual + 1}?`)) avancarFase.mutate();
            }}
      ...
      <div style={sectionStyle}>
        ...
        <div style={progressMeta}>
          Fase {projeto.faseAtual}/{TOTAL_FASES} · {faseLabel} · {bloqueio.cumpridos}/
          {bloqueio.totalRequisitos} na fase atual
        </div>
      </div>
                    ...
                    <div style={{ flex: 1 }}>
                      <div
                        style={{
                          ...
                          color: isFutura ? 'var(--text-muted)' : 'var(--text-primary)',
                        }}
                      >
                        {label}
                      </div>
      ...
      <div style={sectionStyle}>
        ...
        <div style={notesBox}>
          {status.mensagensIA
            ...
            .slice(0, 3)
            .map((msg, i) => (
              <p key={i} style={noteItem}>
                {msg.mensagem}
              </p>
            ))}
          {status.mensagensIA.length === 0 && (
            <p style={{ ...noteItem, fontStyle: 'italic', color: 'var(--text-muted)' }}>
              Nenhuma observação.
            </p>
          )}
        </div>
        ...
        return (
          <div key={req.id} style={phaseDetailRow}>
            <button
              ...
              style={{
                ...checkBox,
                background: done ? 'var(--accent-green-subtle)' : 'var(--bg-secondary)',
                color: done ? 'var(--accent-green)' : 'var(--text-muted)',
                borderColor: done ? 'rgba(105,190,40,0.4)' : 'var(--border)',
              }}
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              <div style={{ display: 'flex', gap: '0.5rem', marginTop: '0.125rem' }}>
                ...
                {req.obrigatorio && (
                  <span style={{ ...metaTag, color: 'var(--sev-critica)' }}>obrigatório</span>
                )}
                {ev && (
                  <span style={{ ...metaTag, color: 'var(--accent-green)' }}>
                    {ev.dataRegistro}
                  </span>
                )}
              </div>
            ...
            {done && (
              <button
                onClick={() => {
                  if (confirm('Remover?')) removerEv.mutate(req.id);
                }}
                style={delBtnSmall}
                title="Remover"
              >
...
function AddAcaoForm({
  ...
  const handleSubmit = () => {
    if (!desc.trim()) return;
    addAcao.mutate(
      { descricao: desc.trim(), responsavelId: resp, dataPrevista: data },
      {
        onSuccess: () => {
          setDesc('');
          onDone();
    ...
    <div style={addFormBox}>
      ...
      <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap' }}>
        ...
        <button
          ...
          disabled={!desc.trim() || addAcao.isPending}
          style={formSubmitBtn}
        >
...
function AcaoRow({
  ...
  const statusColors: Record<string, string> = {
    pendente: 'var(--accent-yellow)',
    executando: 'var(--accent-green)',
    concluido: 'var(--accent-green)',
    atrasado: 'var(--sev-critica)',
  };

  return (
    <div style={acaoRowStyle}>
      <div
        style={{ ...acaoStatusDot, background: statusColors[acao.status] ?? 'var(--text-muted)' }}
      />
      <div style={{ flex: 1, minWidth: 0 }}>
      ...
      <button
        onClick={() => {
          if (confirm(`Remover ação "${acao.descricao}"?`)) removerAcao.mutate(acao.id);
        }}
        style={delBtnSmall}
        title="Remover ação"
      >
...
function AvancoItem({ pend, projectId }: { pend: RequisitoPendente; projectId: string }) {
  ...
  const handleNudge = useCallback(() => {
    ...
    dispatchNudge({
      projectId,
      text: `${firstName}, o líder está aguardando: ${pend.requisito.descricaoCurta}`,
    });
    setSent(true);
  ...
  return (
    <div style={avancoRow}>
      <button onClick={handleMark} style={avancoCheckBtn} title="Marcar como cumprido">
        ☐
      </button>
      ...
      {sent ? (
        ...
        <button
          onClick={handleNudge}
          style={avancoNudgeBtn}
          title={`Cobrar ${pend.responsavelNome}`}
        >
          🔔
        </button>
      )}
...
const STATUS_STYLE: Record<string, React.CSSProperties> = {
  ativo: {
    ...
    borderColor: 'rgba(0,126,122,0.3)',
  },
  atrasado: {
    color: 'var(--sev-critica)',
    background: 'var(--accent-red-subtle)',
    borderColor: 'var(--sev-critica)',
  },
  concluido: {
    color: 'var(--accent-green)',
    background: 'var(--accent-green-subtle)',
    borderColor: 'rgba(105,190,40,0.3)',
  },
};
...
const anoLabel: React.CSSProperties = {
  fontSize: '0.625rem',
  fontWeight: 700,
...
const progressNum: React.CSSProperties = {
  fontSize: '1.5rem',
  fontWeight: 700,
  fontFamily: 'var(--font-mono)',
  color: 'var(--accent-green)',
...
const faseRowStyle: React.CSSProperties = {
  display: 'flex',
  justifyContent: 'space-between',
  alignItems: 'center',
...
const faseNumCircle: React.CSSProperties = {
  width: 32,
  height: 32,
  borderRadius: '50%',
  display: 'flex',
...
// Phase detail
const phaseDetailBox: React.CSSProperties = {
...
const phaseDetailEmpty: React.CSSProperties = {
  padding: '0.75rem 0.5rem 0.75rem 2.75rem',
  fontSize: '0.75rem',
  color: 'var(--text-muted)',
  fontStyle: 'italic',
};
const phaseDetailRow: React.CSSProperties = {
  display: 'flex',
  alignItems: 'flex-start',
  gap: '0.5rem',
...
const avancoBlockStyle: React.CSSProperties = {
  background: 'var(--bg-secondary)',
  border: '2px solid var(--accent-yellow)',
  borderRadius: '6px',
  padding: '1.25rem 1.5rem',
};
const avancoTitleStyle: React.CSSProperties = {
  fontSize: '0.9375rem',
  fontWeight: 700,
  margin: '0 0 0.75rem',
  color: 'var(--accent-yellow)',
};
const avancoRow: React.CSSProperties = {
  display: 'flex',
  alignItems: 'center',
  gap: '0.5rem',
...
const avancoCheckBtn: React.CSSProperties = {
  fontSize: '0.875rem',
  color: 'var(--accent-green)',
  background: 'none',
  border: 'none',
...
const avancoResp: React.CSSProperties = {
  fontSize: '0.6875rem',
  fontWeight: 600,
  color: 'var(--text-muted)',
  flexShrink: 0,
...
const avancoNudgeBtn: React.CSSProperties = {
  width: 30,
  height: 30,
  borderRadius: '6px',
  border: '1px solid var(--border)',
...
const avancoSent: React.CSSProperties = {
  fontSize: '0.625rem',
  fontWeight: 600,
  color: 'var(--accent-green)',
  whiteSpace: 'nowrap',
...
const avancoFooter: React.CSSProperties = {
  fontSize: '0.6875rem',
  color: 'var(--text-muted)',
  marginTop: '0.625rem',
  textAlign: 'center',
...
const avancoReadyStyle: React.CSSProperties = {
  display: 'flex',
  alignItems: 'center',
  gap: '0.75rem',
  padding: '1rem 1.25rem',
...
const addFormBox: React.CSSProperties = {
  display: 'flex',
  flexDirection: 'column',
  gap: '0.5rem',
  padding: '0.75rem',
...
const acaoRowStyle: React.CSSProperties = {
  display: 'flex',
  alignItems: 'flex-start',
  gap: '0.5rem',
  padding: '0.5rem 0.625rem',
...
const acaoStatusDot: React.CSSProperties = {
  width: 8,
  height: 8,
  borderRadius: '50%',
  flexShrink: 0,
